...
const LEGACY_CURRENT_LOCATION_LABELS = new Set([
  "Mevcut Konum",
  "Current Location",
]);
...
function App() {
  ...
  const [locationSearchCache, setLocationSearchCache] = useState<
  ...
  const isSearchableQuery =
    searchQuery.length >= 2 && !LEGACY_CURRENT_LOCATION_LABELS.has(searchQuery);
  ...
  const searchingLocations =
    isSearchableQuery && pendingSearchQuery === normalizedSearchQuery;
  const hasSearchAttempt =
    isSearchableQuery &&
  ...
  const toggleFavoriteLocation = () => {
    ...
    const exists = favoriteLocations.some((item) =>
      isSameLocation(item, selectedLocation),
    );
    if (exists) {
      persistFavorites(
        favoriteLocations.filter(
          (item) => !isSameLocation(item, selectedLocation),
        ),
      );
      return;
    }
  ...
  const useCurrentLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        setError(
          "Location permission was denied or the location could not be retrieved.",
        );
        setLocating(false);
      },
      { enableHighAccuracy: true, timeout: 10000, maximumAge: 300000 },
      ...
      <header className="max-w-7xl mx-auto px-5 md:px-8 pt-6 md:pt-8 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <form onSubmit={onSubmit} className="w-full lg:w-auto">
          <div className="flex gap-2 w-full lg:w-auto lg:min-w-[430px]">
              ...
              {hasSearchInteraction &&
                ...
                (activeSuggestions.length > 0 ||
                  ...
                  <div className="absolute left-0 right-0 top-[calc(100%+10px)] z-30 overflow-hidden rounded-2xl border border-line bg-[rgba(34,27,22,0.96)] text-paper shadow-[0_20px_50px_rgba(34,27,22,0.24)] backdrop-blur-md">
                    {searchingLocations && (
                      <div className="px-4 py-3 text-sm text-paper/70">
                        Searching locations...
                      </div>
                    )}
            ...
            <button
              type="submit"
              className="rounded-2xl bg-ink text-paper px-4 py-3 flex items-center gap-2 shrink-0"
            >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 md:gap-7 items-start">
              ...
              <div className="flex min-w-0 flex-col gap-5 md:gap-6">
                ...
                <ActivityPanel
                  ...
                  activityWindows={data.activity_windows}
                />
...
function normalizeSavedLocation(location: SavedLocation): SavedLocation {
  ...
  return {
    ...location,
    label:
      location.label && LEGACY_CURRENT_LOCATION_LABELS.has(location.label)
        ? "Current Location"
        : location.label,
  };
}
